...
import { Card, Checkbox, H1, H2, H3, H4, ScrollView, Text } from 'tamagui';
...
const QuestItem = ({ quest, onToggleQuest }: any) => {
  ...
  if (Array.isArray(quest.description)) {
    ...
    const current = breakpoints.indexOf(
      breakpoints.find((b: any) => b > data.done)
    );
    ...
    return (
      ...
      >
        <View className="flex items-center w-[90%] gap-1 flex-col  mb-2 mt-2">
          <Text className="mr-2">
...
const QuestItemX = ({ quest, onCheckedChange }: any) => {
  const [checked, setChecked] = useState(false);
  const { data, mutate } = useLocalStorage('exp', 0);
  return (
    <TouchableOpacity
      onPress={() => {
        if (checked) mutate(data - 5);
        else mutate(data + 5);
        setChecked(!checked);
    ...
    >
      <View className="flex flex-row mt-4">
        <Checkbox mr="$2.5" ml="$2.5" size="$4" checked={checked}>
          <Checkbox.Indicator>
            <AntDesign name="check" size={16} color="black" />
          </Checkbox.Indicator>
        </Checkbox>
...
export default function TabOneScreen() {
  const [isExpandedQuests, setIsExpandedQuestes] = useState(false);
  const [isExpandedGoals, setIsExpandedGoals] = useState(false);

  const toggleExpandQuests = () => {
    setIsExpandedQuestes(!isExpandedQuests);
  };

  const toggleExpandGoals = () => {
    setIsExpandedGoals(!isExpandedGoals);
  };

  const [dailiesDone, setDailiesDone] = useState(0);
  const [weekliesDone, setWeekliesDone] = useState(0);

  const { data, mutate } = useLocalStorage('health', 50);

  const { dailyRandomValues, weeklyRandomValue } = useMemo(() => {
    const daily = [];
    for (let i = 0; i < 5; i++) {
      daily.push(Math.floor(Math.random() * QUESTS.daily[i].quests.length));
    }
    const weekly = [];
    for (let i = 0; i < 5; i++) {
      weekly.push(Math.floor(Math.random() * QUESTS.weekly[i].quests.length));
    }
    return { dailyRandomValues: daily, weeklyRandomValue: weekly };
  }, []);

  return (
    <ImageBackground
      source={require('./challenges.png')}
      ...
      <SafeAreaView>
        <ScrollView>
          <View className="px-4 pb-24 gap-4">
            ...
            <TouchableOpacity onPress={toggleExpandQuests}>
            ...
            {isExpandedQuests && (
              <View>
                <Card
                  ...
                  elevate
                  mb="$2"
                  bg={dailiesDone === 5 ? '$green5' : undefined}
                >
                  <H4 marginBottom="$2">Dailies</H4>
                  {QUESTS.daily.map((quest, i) => (
                    <QuestItemX
                      key={quest.categoryTitle}
                      onCheckedChange={(checked: boolean) => {
                        if (checked) setDailiesDone(dailiesDone + 1);
                        else setDailiesDone(dailiesDone - 1);

                        if (weekliesDone === 5) {
                          mutate(data + 1);
                        }
                      }}
                      quest={quest.quests[dailyRandomValues[i]]}
                    />
                  ))}
                </Card>
                ...
                >
                  <H4 marginBottom="$2">Weeklies</H4>
                  {QUESTS.weekly.map((quest, i) => (
                    <QuestItemX
                      key={quest.categoryTitle}
                      quest={quest.quests[weeklyRandomValue[i]]}
                      onCheckedChange={(checked: boolean) => {
                        if (checked) setWeekliesDone(weekliesDone + 1);
                        else setWeekliesDone(weekliesDone - 1);
            ...
            <TouchableOpacity onPress={toggleExpandGoals}>
